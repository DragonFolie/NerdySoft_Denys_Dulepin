...
import example.entity.product.factory.ProductFactory;
import example.util.VirtualProductCodeManager;
...
public class Main {
  public static void main(String[] args) {
    ...
    //TODO Create productFactory that can create a product for a specific type: Real or Virtual
    ...
    var isCodeUsedX = virtualProductCodeManager.isCodeUsed("xxx"); // true
    var isCodeUsedY = virtualProductCodeManager.isCodeUsed("yyy"); // false
    System.out.println("Is code used X: " + isCodeUsedX + "\n");
    System.out.println("Is code used Y: " + isCodeUsedY + "\n");
    // If I am Correct understand task, we need to check that some code which we "useCode", we need to store this or we can check if
    // obj is created by checkInstance but we cant create some code. So i resolve this task by this way, maybe i do wrong
    // This comment only for reviewer, in real case i will never use this comments
  ...
  private static Product getMostExpensiveProduct(List<Order> orders) {
    return Product.getMostExpensiveProductPrice(orders);
  }

  private static Product getMostPopularProduct(List<Order> orders) {
    return Product.getMostPopularProduct(orders);
  }

  private static double calculateAverageAge(Product product, List<Order> orders) {
    return User.calculateAverageAge(product, orders);
  }
